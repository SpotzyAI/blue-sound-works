...
const Hospitality = () => {
  const features = [{
    ...
    items: ["Real-time room availability checking", "Handle bookings, modifications & cancellations", "Manage table reservations for restaurants", "Event space scheduling", "Group booking coordination"]
  }, {
    icon: ConciergeBell,
    title: "Guest Services & Concierge",
    items: ["24/7 virtual concierge assistance", "Local recommendations & directions", "Activity and tour bookings", "Transportation arrangements", "Special request handling"]
  }, {
    icon: MessageSquare,
    title: "Automated Communications",
    items: ["Booking confirmations & reminders", "Check-in/check-out notifications", "Pre-arrival communications", "Post-stay follow-ups & feedback", "Multilingual guest messaging"]
  }, {
    icon: Coffee,
    title: "Dining & Room Service",
    items: ["Restaurant reservations", "Room service orders", "Catering inquiries", "Dietary requirements handling", "Menu information & recommendations"]
  }, {
    icon: Globe,
    title: "Multilingual Support",
    items: ["Serve international guests in 10+ languages", "Natural conversation flow", "Cultural sensitivity", "Accent recognition", "Real-time translation"]
  }, {
    icon: TrendingUp,
    title: "Operations & Analytics",
    items: ["Reduce staff workload by 80%", "Detailed call analytics", "Guest preference tracking", "Peak time optimization", "Revenue enhancement insights"]
  }];
  const useCases = [{
          ...
          <div className="container mx-auto px-6">
            <div className="max-w-6xl mx-auto">
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                {propertyTypes.map((type, index) => <div key={index} className="p-6 rounded-2xl bg-card border border-border shadow-soft hover:shadow-glow transition-all duration-300">
                    <div className="mb-4 p-3 rounded-xl bg-primary/10 text-primary w-fit">
                      <type.icon className="h-6 w-6" />
                    </div>
                    <h3 className="text-xl font-bold mb-4">{type.title}</h3>
                    <ul className="space-y-2">
                      {type.features.map((feature, idx) => <li key={idx} className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <div className="container mx-auto px-6">
            <div className="max-w-6xl mx-auto">
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                {features.map((feature, index) => <div key={index} className="p-6 rounded-2xl bg-card border border-border shadow-soft">
                    <div className="mb-4 p-3 rounded-xl bg-primary/10 text-primary w-fit">
                      <feature.icon className="h-6 w-6" />
                    </div>
                    <h3 className="text-xl font-bold mb-4">{feature.title}</h3>
                    <ul className="space-y-3">
                      {feature.items.map((item, idx) => <li key={idx} className="flex items-start gap-3">
          ...
          <div className="container mx-auto px-6">
            <div className="max-w-6xl mx-auto">
              ...
              <div className="space-y-6">
                {beforeAfter.map((item, idx) => <div key={idx} className="grid md:grid-cols-2 gap-6 p-6 rounded-2xl bg-card border border-border shadow-soft">
          ...
          <div className="container mx-auto px-6">
            <div className="max-w-6xl mx-auto">
              ...
              <div className="grid md:grid-cols-3 gap-8">
                {useCases.map((useCase, idx) => <div key={idx} className="p-6 rounded-2xl bg-card border border-border shadow-soft">
                    <h3 className="text-xl font-bold mb-6 flex items-center gap-2">
                      <Phone className="h-5 w-5 text-primary" />
                      {useCase.title}
                    </h3>
                    <div className="space-y-4">
                      {useCase.conversation.map((msg, msgIdx) => <div key={msgIdx} className={`p-4 rounded-xl ${msg.speaker === "Guest" ? "bg-muted" : "bg-primary/10 border border-primary/20"}`}>
              ...
              <div className="grid md:grid-cols-2 gap-8">
                <div className="p-8 rounded-2xl bg-gradient-hero text-primary-foreground">
                  ...
                  <ul className="space-y-4">
                    {["Complete project setup and configuration", "Custom AI assistant trained for your property", "Seamless PMS integration support", "Ongoing maintenance and updates", "Technical support and training", "Analytics and performance reports", "Dedicated account manager"].map((item, idx) => <li key={idx} className="flex items-start gap-3">
                ...
                <div className="p-8 rounded-2xl bg-card border border-border shadow-soft">
                  ...
                  <ul className="space-y-4">
                    {["Zero missed booking opportunities", "Consistent guest experience", "Reduced operational costs", "Multilingual guest support", "Scalable across all properties", "Data-driven insights"].map((item, idx) => <li key={idx} className="flex items-start gap-3">
